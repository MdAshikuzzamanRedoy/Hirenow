...
<html>
 ...
 <center> <body>
    <div class="bgimage">
      <div class="menu">
        <div class="leftmenu">
          <h3><a href="index.html">Hire_NOw</a></h3>
        </div>
        <div class="rightmenu">
          <ul>
...
<form class="box" action="../Controller/admincontroller.php"onsubmit="return validation()" method="post">
	  ...
	  <input type="text"id="spec" name="spec" placeholder="specility" />
      <input type="submit" name="employeesignup" value="Sign-Up" />
    </form>
...
function validation(){
      	...
      	var uid = document.getElementById('uname').value;
			...
			if(nid.length!=15){
				document.getElementById('error_message').innerHTML =" ** nid Number must be 15 digits only";
				return false;
			}
...
if(spec == ""){
				document.getElementById('error_message').innerHTML =" ** Please fill the specility field";
				return false;
			}
if((spec.length <=3) || (spec.length  >20)) {
				document.getElementById('error_message').innerHTML =" ** specility must be between 3 to 20";
				return false;
			}
