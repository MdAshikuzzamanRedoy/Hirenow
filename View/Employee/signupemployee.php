...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  if(isset($_POST['submit']))
  {
    ...
    if(empty($_POST['wsec']))
    {
      $w_err="Working Sectior CAN NOT BE EMPTY";
    $flag = 0;
    }else{
      $wsec=$_POST['wsec'];
    }
...
<center><form class="box" method="post" action="<?php htmlspecialchars($_SERVER['PHP_SELF'])?>" name="registration">
      ...
      <h1>Registration For Employee</h1>
         ...
         <input type="text" placeholder='Work experience if (yes)' name="wexp" value="<?php echo $wexp;?>" ></td><br>




       <input type="text" placeholder='Working Sectors' name="wsec" value="<?php echo $wsec;?>" ></td>


     <td id="w_err"><?php echo  $w_err;?></td><br><br>
